...
class VGG(nn.Module):
    ...
    def __init__(self, builder, features, last2dim=512):
        super(VGG, self).__init__()
        self.features = features
        num_classes = 10 if parser_args.set == "CIFAR10" else 100
        self.linear = builder.conv1x1(last2dim, num_classes)
        self.wl = []
...
def make_layers(cfg, builder, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            ...
            if batch_norm:
                layers += [conv2d, nn.ReLU(inplace=True), nn.BatchNorm2d(v, eps=1e-5)]
            else:
...
def _vgg_custom(cfg, batch_norm, builder):
    model = VGG(builder, make_layers(cfg, builder, batch_norm=batch_norm), last2dim=cfg[-2])
    return model
